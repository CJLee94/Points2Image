...
def compute_metrics(pred_labeled, gt_labeled, names):
    """
    ...
    if 'p_dice' in names:
        results['p_dice'] = pixel_dice(gt_binary, pred_binary)

    if 'p_iou' in names:
        results['p_iou'] = pixel_iou(gt_binary, pred_binary)
...
def pixel_dice(gt, pred):
    ...
    total_pred = np.count_nonzero(predict)
    total_true = np.count_nonzero(mask)

    dice_value = float(2 * tp) / (total_pred + total_true + 1e-9)
...
def pixel_iou(gt, pred):
    ...
    # print('np nonzero tp', tp)
    # print('np nonzero union', pred_true_union)
    # tp_cv2 = cv2.countNonZero(dice_map == 3)
    # pred_true_union_cv2 = cv2.countNonZero(dice_map)
    # print('cv2 nonzero tp', tp_cv2)
    # print('cv2 nonzero union', pred_true_union_cv2)
...
def AJI_fast(gt, pred_arr):
    ...
    # import pdb
    # pdb.set_trace()
    if not np.all(gs == np.arange(len(gs))):
        tmp_gt = gt.copy()
        for label_from, label_to in zip(gs, np.arange(len(gs))):
            tmp_gt[gt==label_from] = label_to
        gt = tmp_gt
