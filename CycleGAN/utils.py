...
def iou_on_batch(masks, pred):
    ...
    for i in range(pred.shape[0]):
        ...
        # import pdb
        # pdb.set_trace()
        ious.append(iou_numpy(pred_tmp.astype(bool), mask_tmp.astype(bool)))
    return np.mean(ious)
...
def save_on_batch(images1, masks, pred, names, vis_path):
    ...
    for i in range(pred.shape[0]):
        ...
        # import pdb
        # pdb.set_trace()
        cv2.imwrite(os.path.join(vis_path, 'Sample_'+str(names[i].item()) + "_pred.jpg"), pred_tmp)
        cv2.imwrite(os.path.join(vis_path, 'Sample_'+str(names[i].item()) + "_gt.jpg"), mask_tmp*255)
